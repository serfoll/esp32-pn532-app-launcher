...
- **Game launching**: prefers a launcher protocol (`steam://rungameid/<appid>`, detected by reading the Steam-written `appmanifest_*.acf` next to the install folder) over spawning the exe directly, since many launcher-installed games are DRM-wrapped stubs that exit silently outside their launcher (that includes exiting normally after handing off to their launcher — confirmed live with EA App, so `launch_game` no longer treats an immediate exit as a failure signal, it's indistinguishable from a working hand-off). Falls back to a direct exe spawn for anything not installed through a detected launcher. Before either path, checks whether anything is already running from the game's install *folder* (`sysinfo` crate, case-insensitive, component-wise path match — Windows paths are case-preserving but case-insensitive, and `current_exe()`/sysinfo's reported path can differ only in case for the same file) so a cart pulled and reinserted while its game is open doesn't spawn a second instance. Checked by folder rather than the exact recorded exe: confirmed live that EA App's stub hands off to a different exe in the same folder and exits the stub by design, so an exact-exe check would see the stub gone and relaunch anyway. `open` crate to invoke the OS's protocol handler.
- **Background/tray behavior**: closing the window doesn't quit the app by default — the app's whole point is watching the reader in the background. First close intercepts the OS close request (`WindowEvent::CloseRequested` + `CloseRequestApi::prevent_close`), emits an event the frontend answers with a dialog (minimize to tray vs. quit, with an optional "remember my choice" persisted as `settings.closeBehavior`: `null` = ask next time, `true` = minimize, `false` = quit). A tray icon (`tauri`'s built-in `tray-icon` feature, no separate plugin) with Show/Quit menu items is the way back once the window is hidden — without it, hiding would strand the user with no path back except Task Manager.
- **Custom title bar**: `decorations: false` in `tauri.conf.json`, with a hand-built title bar in `index.html`/`main.ts` (`data-tauri-drag-region` for window dragging, `@tauri-apps/api/window`'s `getCurrentWindow()` for the controls). The "Cart Reader" title text doubles as a Steam-style dropdown menu trigger (Settings/Logs/Exit). Window-manipulation plugin commands aren't covered by `core:default` and need explicit grants in `capabilities/default.json` (`allow-minimize`, `allow-toggle-maximize`, `allow-close`, `allow-hide`). The native `minimize()` call corrupts the window on Windows once decorations are disabled (confirmed live: window collapses to a 16×16 rect and never recovers) — the minimize button calls `hide()` instead, reusing the same tray-hide path the close-to-tray flow already relies on. `toggleMaximize()` and `close()` work fine natively.
